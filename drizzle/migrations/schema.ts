import { pgTable, foreignKey, text, timestamp, unique, boolean, integer, doublePrecision } from "drizzle-orm/pg-core"
...
export const sidekickSetting = pgTable("sidekick_setting", {
	...
	systemPrompt: text("system_prompt").default('You are a friendly, professional assistant focused on qualifying leads and helping with business inquiries.').notNull(),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow(),
}, (table) => [
	foreignKey({
